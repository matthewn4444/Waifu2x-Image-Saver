...
exports.convertToPng = function(tab, imageData, callback) {
    if (!imageData.isTransparencyCalculated()) {
        return callback(new Error("Trying to convert to png without calculating transparency"));
    }
    let opt = {
        data: imageData.toDataURL(),
    };
    ensureCalled("resize", opt, 0, tab, function(res, options) {
       if (res.error) {
            return callback(res.error);
        }
        callback(null, new ImageBinary(res.data,
