...
function WaifuImage(tab, src, imageWidth, imageHeight, options) {     // TODO choose server based on which is working, also allow force upload mode
    // Parse the filename and extension from the source url
    let pathname = urlUtils.URL(src).pathname;
    let end = pathname.lastIndexOf(".");
    if (end == -1) return this.__finish("Source path name is wrong");
    let start = pathname.lastIndexOf("/", end);
    let filename = start == -1 ? pathname.substring(0, end) : pathname.substring(start + 1, end);

    // Get the extension, skip upscale by url if extension has extra characters like ".jpg:original"
    let extText = pathname.substring(end + 1);
    let sourceExt = /^([^\W_]+)\W*/.exec(extText)[1];
    if (!sourceExt) {
        // Fallback, this will be likely to fail
        sourceExt = extText;
    }
    ...
    this.__isPng = sourceExt == "png";
    this.__skipUpscaleByUrl = extText != sourceExt || this.__isPng;
    this.__tmpFile = OS.Path.join(OS.Constants.Path.tmpDir, filename + "." + sourceExt);
    this.__completeCallback = null;
    this.__canUpscaleAgain = options.upscaleOnce                         // Allow more than 1 upscale
            && (options.upscaleOnceMaxArea == 0                          // Upload area is 0
            || imageWidth * imageHeight > options.upscaleOnceMaxArea);   // If the image is larger than max area to upload once

    // Other Options
    this.__compression = options.compressionPercentage || 0;
}

WaifuImage.prototype.upscale = function(callback) {
    this.__completeCallback = callback;
    ...
    if (Waifu2xBooru.isLargeEnough(this.width, this.height)) {
        ...
        this.__finish();
    } else if (this.__skipUpscaleByUrl) {
...
WaifuImage.prototype.__resizeAndUpscale = function(e) {
    if (!this.__binaryData) {
        ...
        if (size > Waifu2xBooru.MAX_SIZE) {
            ...
            imageUtils.compress(this.__binaryData, 0, Waifu2xBooru.MAX_SIZE,
...
WaifuImage.prototype.__finish = function(e) {
    // Allow the image to be upscaled again if allowed
    if (e == null && this.__canUpscaleAgain) {
        console.log("   Upscale again!", this.src);
        this.__waifu2xUpscale();
        return;
    }

    // Compress image if allowed
    if (this.__compression > 0 && !this.__binaryData.isTransparent()) {
        console.log("   Compressing image from settings: ", this.__compression);
        imageUtils.compress(this.__binaryData, this.__compression, Waifu2xBooru.MAX_SIZE,
                Waifu2xBooru.MAX_SIDE, this.__finish.bind(this));
        this.__compression = 0;
        return;
    }
    ...
    if (this.__completeCallback) {
        let transparent = this.__binaryData ? this.__binaryData.isTransparent() : false;
        this.__completeCallback.call(this, e, transparent, function(destinations) {
            if (!e) {
                // Save the image
                if (this.__binaryData) {
                    let promises = [];
                    for (let i = 0; i < destinations.length; i++) {
                        let path = destinations[i];
                        console.log("   Saving image to...", path);
                        let promise = this.__binaryData.save(path);
                        promise.catch(console.log);
                        promises.push(promise);
                    }
                    Promise.all(promises).then(errors => {
                        this.__binaryData = null;
                        console.log("   Finished saving images", this.src);
                    });
                }
            }
        }.bind(this));
        ...
        console.log("   Failed no complete callback");
    }
}
...
WaifuImage.prototype.__waifu2xUpscale = function() {
    ...
    if (Waifu2xBooru.needsResize(this.width, this.height)
            || (this.__binaryData && this.__binaryData.length > Waifu2xBooru.MAX_SIZE)) {
        this.__resizeAndUpscale();
        return;
    }
    ...
    if (this.__binaryData) {
        ...
        let requestPng = this.__binaryData.isTransparent()
             || !this.__canUpscaleAgain && this.__isPng && !this.__compression > 0;
        ...
        this.__request.upscaleByUpload(this.__binaryData, requestPng, function(e, data, savedAsPng) {
            ...
            if (e) {
                ...
                // If requestPng but returns as jpg, then it needs to be converted back to png
                if (requestPng && !savedAsPng) {
                    if (this.__binaryData.isTransparent()) {
                        // Use the map to add back the opacity
                        ...
                        // Just resave it as png
                        console.log("   Received jpg back, convert back to png");
                        imageUtils.convertToPng(this.tab, data, function(e, data) {
                            if (e) return this.__finish(e);
                            this.__postUpscale(data);
                        }.bind(this));
                    }
