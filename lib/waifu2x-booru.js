...
class Request {
    ...
    __execute() {
       ...
       (this.__action == "POST" ? utils.post(this.__url, this.__data) : utils.get(this.__url))
...
function upscaleByUpload(blob, saveAsPng /* optional */, denoise /* optional */,
        scale /* optional */) {
    let data = {
        denoise: denoise || 1,
    ...
    return new Request(UploadUrl, "POST", opts);
}

function upscaleByUrl(url, saveAsPng /* optional */, denoise /* optional */, scale /* optional */) {
    // Generate the url
    let uploadUrl = FromLinkUrl + "?url=" + encodeURIComponent(url) + "&denoise=" + denoise
        + "&scale=" + (scale === false ? "1" : "2") + "&submit=";
    return new Request(uploadUrl, "GET", { saveAsPng: saveAsPng || false });
}
