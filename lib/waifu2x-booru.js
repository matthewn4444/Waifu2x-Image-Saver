...
Request.prototype.upscaleFromUrl = function(url, saveAsPng, callback) {
    ...
    let noise = prefs.getNoiseLevel() == 0 ? "" : "&denoise=" + prefs.getNoiseLevel();
    this.__url = "https://waifu2x.booru.pics/Home/fromlink?url=" + encodeURIComponent(url) + noise + "&scale=" + scale + "&submit=";
...
Request.prototype.__checkStatus = function(handle, callback) {
    // Do not cancel here because server thinks it is still going
