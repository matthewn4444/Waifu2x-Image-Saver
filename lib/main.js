...
const MAX_UPLOAD_AREA = 1280 * 1280;
const MAX_SMALL_IMAGE = MAX_UPLOAD_AREA / 4;
const MAX_ACCEPTED_AREA = MAX_UPLOAD_AREA * 4 * 0.8;
const SCALE_THRESHOLD_1_6 = 0.95;
    ...
    onMessage: function(message) {
        ...
        utils.openSaveAsDialogForImages(LABEL, src, function(jpgSavePath, filename, extension) {
            ...
            let data = {
                ...
                isSmallImage: area <= MAX_SMALL_IMAGE
            };
            ...
            Task.spawn(function() {
                ...
                if (area > MAX_UPLOAD_AREA) {
                    ...
                    if (prefNonCrossDomainSet.has(host)) {
                        ...
                        let scaleFactor = getScaleFactorForUpsize(data);
                        waifu2x.upscaleAndSave(src, tmpFile1, scaleFactor, function(e) {
                            if (e) {
                                ...
                                data.width = Math.floor(data.width * scaleFactor);
                                data.height = Math.floor(data.height * scaleFactor);
                                compressImage(data);
                            }
                        });
                    }
...
|*      Helper Functions      *|
\*============================*/
function isRecursionForSizeAllowed(data) {
    return !prefs.isRecursionPrevented() && (data.width * data.height) < MAX_ACCEPTED_AREA
            && (prefs.allowSmallImagesRecursiveUpscale() || !data.isSmallImage);
}

function getScaleFactorForUpsize(data) {
    if (isRecursionForSizeAllowed(data)) {
        // Check if area is smaller than upload area, and when 1.6x will it be over
        let area = data.width * data.height;
        let threshold = MAX_UPLOAD_AREA * SCALE_THRESHOLD_1_6;
        if (area < threshold && area * 1.6 >= threshold) {
            return waifu2x.SCALE_1_6;
        }
    }
    return waifu2x.SCALE_2;
}

function uploadThenCompressImage(data, callback) {
    var scaleFactor = getScaleFactorForUpsize(data);
    waifu2x.uploadAndUpscaleImage(data.tmp1, scaleFactor, function(e) {
        if (e) {
            OS.File.remove(data.tmp1);
            handleFailure(e, data.filename + ".jpg", data.download, data.list);
        } else {
            data.width = Math.floor(data.width * scaleFactor);
            data.height = Math.floor(data.height * scaleFactor);
...
function compressImage(data, callback) {
    ...
    if (code == 0) {
        ...
        if (isRecursionForSizeAllowed(data, area)) {
