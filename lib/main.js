...
function notify(text) {
    notifications.notify({
        ...
        text: text,
        iconURL: self.data.url("/images/icon-64.png")
    });
}
            ...
            Task.spawn(function() {
                ...
                waifu2x.upscaleAndSave(src, tmpFile1, function(e) {
                    if (e) {
                        ...
                        let message = e.toString().indexOf("image size exceeds maximum") ?
                            "the image is too big" : "image is not supported";
                        notify("Cannot download image: " + message);
                        console.error(e);
                    } else {
                        // Compress the image
                        utils.compressImage(tmpFile1, tmpFile2, function(code) {
                            ...
                            if (code == 0) {
                                ...
                                notify("Cannot download image: compression failed to run correctly");
